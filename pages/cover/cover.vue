  ...
  <view v-if="internalAccessGranted" class="page">
    <scroll-view class="content" scroll-y :lower-threshold="100" @scrolltolower="loadMoreDesignLibrary">
      ...
      <template v-if="activeTab === 'workflow'">
        ...
        <view class="section compact"><view class="field-title"><text>*</text>选择保存路径</view><view class="save-target-control"><view class="save-target" @click="openSaveTarget"><text :class="{ placeholder: !selectedSaveTargetLabel }">{{ selectedSaveTargetLabel || '请选择保存路径' }}</text><text class="save-target-arrow" :class="{ open: saveTargetVisible }">⌄</text></view><save-target-sheet :visible="saveTargetVisible" :scopes="saveTargetScopes" :value="activeDraft.save_target" :loading="saveTargetsLoading" :error="saveTargetsError" @confirm="confirmSaveTarget" @close="closeSaveTarget" /></view><text v-if="saveTargetsError" class="sync-note">{{ saveTargetsError }}</text><text v-if="draftSyncIssue" class="sync-note">草稿暂未同步到账号，请稍后重试。</text></view>
...
export default {
  ...
  methods: {
    ...
    async openSaveTarget() {
      if (this.saveTargetVisible) { this.closeSaveTarget(); return }
      this.saveTargetVisible = true
